# -*- coding: utf-8 -*-
"""
Publish the Merged — Thuwaraga Products dashboard to tech_team_outputs.ph_task.
One guarded row per user, stable task_id (upsert via --refresh), md5 read-back.

  python publish_thuwaraga_ph_task.py --dry-run
  python publish_thuwaraga_ph_task.py --team        # INSERT one row per user
  python publish_thuwaraga_ph_task.py --refresh     # UPDATE html+name of all tpts rows (automation)
"""
import os, sys, argparse, hashlib, psycopg2

HERE = os.path.dirname(os.path.abspath(__file__))
HTML_FILE = os.path.normpath(os.path.join(HERE, "..", "thuwaraga_t7_smaw",
                                          "merged_thuwaraga_dashboard.html"))
DB_CONFIG = {
...
PROJECT_NAME = "Merged — Thuwaraga Products"
PROJECT_CODE = "tpts"
TASK_NAME    = "Merged — Thuwaraga Products (SKU Performance + Amazon Stock)"
DESCRIPTION  = ("One page combining two Thuwaraga per-SKU reports — Weekly SKU Performance (T7) and "
                "Table-5 Amazon Stock Check (SMAW). Pick a task tab to see that task's own SKUs; search, "
...
def task_id_for(user):
    return f"tpts_{user}_thuwaraga_products"
def _require():
