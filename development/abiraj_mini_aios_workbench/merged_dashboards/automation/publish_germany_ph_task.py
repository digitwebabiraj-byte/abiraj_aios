# -*- coding: utf-8 -*-
"""
Publish the Germany Products (Fast & Slow) merged dashboard to tech_team_outputs.ph_task.
One guarded row per German PH user, stable task_id (upsert via --refresh), md5 read-back.

  python publish_germany_ph_task.py --dry-run
  python publish_germany_ph_task.py --team        # INSERT one row per German PH user
  python publish_germany_ph_task.py --refresh     # UPDATE html of all gpfs rows (automation)
"""
import os, sys, argparse, hashlib, psycopg2

HERE = os.path.dirname(os.path.abspath(__file__))
HTML_FILE = os.path.normpath(os.path.join(HERE, "..", "germany_products_fmp_smp",
                                          "merged_germany_products_dashboard.html"))
DB_CONFIG = {
...
AUDIENCE = ["Mahima"]     # FMP + SMP are Mahima's. Override with --users a,b,c
...
def task_id_for(user):
    return f"gpfs_{user}_germany_products_fast_slow"
def _require():
