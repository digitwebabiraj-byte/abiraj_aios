@echo off
REM Germany Products merge monthly run — refresh live (FMP + SMP) -> build -> publish to Mahima.
REM Credentials from the GLOBAL user environment (LED_PGPASSWORD + PGPASSWORD); non-secret defaults below.
setlocal
cd /d "%~dp0"
...
python "%~dp0merge_germany_run.py" %* >> "%~dp0merge_germany_run.log" 2>&1
set RC=%ERRORLEVEL%
if not "%RC%"=="0" echo [MERGE-DE] run FAILED with exit code %RC% - see merge_germany_run.log >&2
exit /b %RC%
