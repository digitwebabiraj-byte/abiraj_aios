# -*- coding: utf-8 -*-
"""
Thuwaraga Products merge — live monthly run (T7 + SMAW, Thuwaraga).
Reuses each task's OWN build (live WAREHOUSE pull, NO publish via --dry-run) to produce fresh
data, reads it via the emitters, builds the dashboard, republishes the thuwaraga rows. Both tasks
use the warehouse (5435) — no PGPORT stripping needed. Task folders restored via git checkout.
...
T7_SQL = os.path.join(T7_PROJ, "sql", "T7_weekly-sku-performance-check", "generate_dataset.sql")  # canonical query
SMAW_PROJ = os.path.join(PROJECTS, "PRJ-2026-004_smaw-table5-stock-check")
SMAW_RUN_DIR = os.path.join(SMAW_PROJ, "automation")             # smaw_weekly_run.py --dry-run = live, no publish
...
def fetch_t7(workdir):
    # Reuse T7's OWN canonical query (generate_dataset.sql, %(ws)s/%(we)s = rolling 7d) against the
    # warehouse read-only, and write a fresh data.json into the workdir. Does NOT touch the T7 task.
...
def main():
    ...
    try:
        t7_json = fetch_t7(work)                                              # live warehouse pull, temp only
        run_build("SMAW", SMAW_RUN_DIR, "smaw_weekly_run.py", ["--dry-run"])  # live warehouse pull, no publish
