...
**V2 → V3 UI (same day):** V2 moved the per-PH dashboards to an embed-friendly document-flow layout
(fixes the cramped internal-scroll box in the portal). V3 (owner feedback) rebuilt them as a
**full-width, full-height data table** (Status · Product · Units · Returns · Rate · Reasons · Likely
cause · Recommended action) with a sticky toolbar + sticky column header, and trimmed the redundant top
KPI tiles + sidebar mini-stats down to one severity strip + reason legend. Each republished as a
guarded in-place UPDATE of the same 19 rows (`version_level` now 3), identity fields unchanged,
md5-verified via read-only MCP.
