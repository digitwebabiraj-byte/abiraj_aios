...
#  Runs run_frrc_monthly.bat on day 8 of every month at 09:00.
...
# --- create: day 8 of every month, 09:00 ---
# /IT = run only when this user is logged on -> no Windows password needs storing.
schtasks.exe /Create /TN $TaskName /TR "`"$Bat`"" /SC MONTHLY /D 8 /ST 09:00 /RU "$env:USERNAME" /IT /F | Out-Null
...
Write-Host "Registered '$TaskName' - day 8 of every month, 09:00." -ForegroundColor Green
