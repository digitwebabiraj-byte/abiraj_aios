...
Set-Content -Path $alert -Encoding utf8 -Value @(
  ...
  ('Log : ' + (Join-Path $Dir 'pc_run.log')),
  ('Re-run once the DB is healthy : ' + (Join-Path $Dir 'run_pc_weekly.bat')),
  '',
  'This file clears itself automatically after the next successful run.'
)
try { msg * /TIME:0 'PC weekly paused-campaign run FAILED - see PC_ALERT_FAILED.txt on your Desktop.' } catch {}
exit 0
