@echo off
REM CHOP - Channel Opportunity monthly run. Live pull (ledsone) -> build -> guarded ph_task upsert.
REM Credentials come from the GLOBAL user environment (LED_PGPASSWORD + PGPASSWORD); non-secret
REM coordinates default below. Passing no password anywhere -> the .py aborts before writing.
setlocal
cd /d "%~dp0"
...
python "%~dp0chop_monthly_run.py" %* >> "%~dp0chop_run.log" 2>&1
set RC=%ERRORLEVEL%
if not "%RC%"=="0" echo [CHOP] run FAILED with exit code %RC% - see chop_run.log >&2
exit /b %RC%
