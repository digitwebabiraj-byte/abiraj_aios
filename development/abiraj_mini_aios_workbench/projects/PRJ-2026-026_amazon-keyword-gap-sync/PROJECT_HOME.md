...
| **Automation** | ✅ **REQ-30-D03 — AUTOMATED 2026-08-19.** Windows task `BGCT_Monthly_Keyword_Gap`, **20th of each month 12:00**, registered on the main-tree path, **proven end-to-end (`LastTaskResult 0`, 776 bulbs, 136 gaps, last-good md5 == the DB's)**. Fail-closed: catalogue floor 500 · accounting-ties assertion · 60% collapse guard · Top-Movers ≥ 1 · SQP-not-absent · no-silent-account-loss · render ≥ 150 KB → refuse, leave id 980 untouched, alert. **Deliberately NO minimum on the gap count** — the gap count is a backlog that should trend to zero as she does the work, so a floor would fail the job exactly when the project succeeds. See `automation/AUTOMATION_README.md`. ⚠ The rules it re-publishes each month are still **unconfirmed defaults**. |
