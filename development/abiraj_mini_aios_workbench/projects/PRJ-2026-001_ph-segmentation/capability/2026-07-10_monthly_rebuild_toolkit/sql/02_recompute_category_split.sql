...
ccls AS (SELECT a.ref_id,a.sub_source_name,a.imp,a.clk,a.conv,a.cvr,b.bi,b.bk,b.bv,(CASE WHEN a.imp>=b.bi THEN 'H' ELSE 'L' END)||(CASE WHEN a.clk>=b.bk THEN 'H' ELSE 'L' END)||(CASE WHEN a.clk=0 THEN 'L' WHEN a.conv>a.clk THEN 'H' WHEN a.cvr>=b.bv THEN 'H' ELSE 'L' END) rawc FROM ca a CROSS JOIN cbm b),
...
pse AS (SELECT a.ref_id,a.imp,a.clk,a.cvr,ROW_NUMBER() OVER (ORDER BY u.u DESC,a.imp DESC) rnk,COUNT(*) OVER () scnt FROM psig a JOIN pu u ON a.ref_id=u.asin AND u.u>0),
pbm AS (SELECT AVG(imp) FILTER (WHERE rnk<=CASE WHEN scnt>=30 THEN 30 ELSE 10 END) bi,AVG(clk) FILTER (WHERE rnk<=CASE WHEN scnt>=30 THEN 30 ELSE 10 END) bk,AVG(cvr) FILTER (WHERE rnk<=CASE WHEN scnt>=30 THEN 30 ELSE 10 END) bv FROM pse),
pcls AS (SELECT a.ref_id,a.sub_source_name,(CASE WHEN a.imp>=b.bi THEN 'H' ELSE 'L' END)||(CASE WHEN a.clk>=b.bk THEN 'H' ELSE 'L' END)||(CASE WHEN a.clk=0 THEN 'L' WHEN a.conv>a.clk THEN 'H' WHEN a.cvr>=b.bv THEN 'H' ELSE 'L' END) rawc FROM psig a CROSS JOIN pbm b),
