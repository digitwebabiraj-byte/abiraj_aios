-- ============================================================================
-- PH ASIN SEGMENTATION — MONTHLY ENGINE  (last 4 COMPLETE weeks; COUNT-based conversion)
-- ***** CONVERSION RULE CHANGE — Bietrick-approved 2026-07-10 *****
-- The 3rd signal (Conversion) is now scored the SAME way as Impressions and Clicks:
--   Conversion = HIGH if  a.conv >= b.bcv  (ASIN sales COUNT >= category Avg conversions), else LOW.
-- This REPLACES the old CVR-RATE rule (was: zero-click->L ; conv>clk->H ; cvr>=bench-cvr->H else L).
-- Effect: segment now agrees with the count-based Status column; some HHL "Leaky Buckets" become
-- HHH "Champions" (high traffic + good sales count even at a weaker rate). STORED, NOT EXECUTED here —
-- re-run + HTML regenerate + live push happen in the authorised DB session. The undefined-combo map
-- (HLL->HLH, LHL->HHL) is UNCHANGED and remains a separate open decision.
-- ----------------------------------------------------------------------------
-- ----------------------------------------------------------------------------
...
--
-- LOCKED DECISIONS: benchmark still uses Method A CVR + avg counts over top-N sold;
-- Conversion signal = COUNT-based (conv >= avg-conv), per the change note above ;
...
-- (top-10 if <30 ; needs_manual if <10) ; movement by STRICT segment rank
-- (HHH=1, HHL=2, HLH=3, LHH=4, LLH=5, LLL=6 ; lower rank = better performance) ;
-- account from sub_source_name ; ownership read from user_name (orphans,
-- user_name NULL, are excluded here and surfaced separately by
-- analytics.v_orphan_asins).
--
-- MOVEMENT / NEW RULE (strict-rank, returning-aware, equal 4-week windows):
--   IMPROVED = current rank < previous rank (moved to a better segment) ;
--   DECLINED = current rank > previous rank ; SAME = equal rank.
--   (HHL/HLH/LHH are DISTINCT ranks 2/3/4, not tied -- this is the strict rule
--    signed off 2026-07-02, replacing the earlier equal-weight h-count.)
...
raw AS (
  SELECT a.user_name,a.ref_id,a.category_name,a.sub_source_name,a.imp,a.clk,a.conv,a.cvr,
    b.bi,b.bc,b.bcv,b.bv,b.sellers,(b.bi IS NULL OR b.sellers<10) needs_manual,
    (CASE WHEN a.imp>=b.bi THEN 'H' ELSE 'L' END)
    ||(CASE WHEN a.clk>=b.bc THEN 'H' ELSE 'L' END)
    ||(CASE WHEN a.conv>=b.bcv THEN 'H' ELSE 'L' END) rawc
...
SELECT user_name,ref_id,category_name,
  ...
  sub_source_name,imp,clk,conv,cvr,bi,bc,bcv,bv,sellers,needs_manual,rawc,
  CASE rawc WHEN 'HLL' THEN 'HLH' WHEN 'LHL' THEN 'HHL' ELSE rawc END segment,
  -- STRICT segment rank (1=best .. 6=worst): HHH<HHL<HLH<LHH<LLH<LLL
  CASE CASE rawc WHEN 'HLL' THEN 'HLH' WHEN 'LHL' THEN 'HHL' ELSE rawc END
    WHEN 'HHH' THEN 1 WHEN 'HHL' THEN 2 WHEN 'HLH' THEN 3 WHEN 'LHH' THEN 4 WHEN 'LLH' THEN 5 ELSE 6 END srank,
  (SELECT max(date) FROM weeks) AS window_end,
  (SELECT min(date)-6 FROM weeks) AS window_start
FROM raw;

-- ============================================================================
-- PREVIOUS window = weeks rn 5..8  (same logic; only ref_id/segment/srank kept)
-- ============================================================================
...
SELECT ref_id,sub_source_name,
  ...
  -- STRICT segment rank (1=best .. 6=worst): HHH<HHL<HLH<LHH<LLH<LLL
  CASE CASE rawc WHEN 'HLL' THEN 'HLH' WHEN 'LHL' THEN 'HHL' ELSE rawc END
    WHEN 'HHH' THEN 1 WHEN 'HHL' THEN 2 WHEN 'HLH' THEN 3 WHEN 'LHH' THEN 4 WHEN 'LLH' THEN 5 ELSE 6 END prev_srank
FROM raw;
...
SELECT TO_CHAR(date_trunc('month',(SELECT pe FROM cur_end)) + INTERVAL '1 month','YYYY-MM') AS report_period,
  ...
  c.bi::numeric(10,0) bm_impression,c.bc::numeric(10,1) bm_click,c.bcv::numeric(10,2) bm_conversion,(c.bv*100)::numeric(10,2) bm_cvr_pct,
  c.sellers benchmark_sellers,c.needs_manual,c.segment,p.prev_segment,
  CASE
    WHEN p.prev_segment IS NOT NULL THEN
      -- strict rank: LOWER number = better, so current<previous means IMPROVED
      CASE WHEN c.srank<p.prev_srank THEN 'IMPROVED'
           WHEN c.srank>p.prev_srank THEN 'DECLINED' ELSE 'SAME' END
